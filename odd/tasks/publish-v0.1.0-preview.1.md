...
- [ ] 9. **Refresh the release-preparation branch and apply the notice remediation.** From the updated default branch after both lifecycle merges, create a fresh isolated release worktree. Reapply or merge the reviewed preparation work without rewriting published history, resolve only observed conflicts, and add the separately reviewed root `THIRD_PARTY_NOTICES` required by F-05, reproducing the `golang.org/x/sys v0.38.0` `LICENSE` and `PATENTS` obligations. Confirm the changelog, runbook, ODD evidence, root Apache-2.0 `LICENSE`, third-party notice, canonical/legacy CLI behavior, active residual SDD change, and evidence-only language remain truthful. Update release documentation so every Linux archive has exactly four members—`gce`, `git-change-evidence`, `LICENSE`, and `THIRD_PARTY_NOTICES`—while the external upload set remains six unsigned files. **Depends on:** 5 and 8. **Evidence:** pending branch base/head/tree, retained work-unit identities or replacement mapping, notice source/comparison evidence, changed paths/LOC, and clean status. **Recovery:** discard the release worktree and repeat from the merged base; never force-update a shared ref without explicit authority.
...
- [ ] 13. **Freeze the exact final release commit and refresh automated audits.** Select one immutable default-branch commit as the only candidate for tag `v0.1.0-preview.1`. Re-run Task 1's automated audit and Task 10's verification against that exact commit, including the reviewed `THIRD_PARTY_NOTICES`, four-member Linux archive contract, six-file external asset contract, hosting settings, and tag/release/package absence. Produce a final publication manifest binding commit, tree, audit/disposition revisions, CI, lifecycle archives, asset contract, limitations, and owner decisions. Keep the repository private. **Depends on:** 12. **Evidence:** pending final commit/tree, audit/report hashes, notice hash, CI, manifest, and clean reproducibility readback. **Recovery:** any source or setting change invalidates the freeze and returns to the applicable earlier task.
